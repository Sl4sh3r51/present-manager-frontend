...
function handleSubmit() {
  ...
  if (form.value.type !== 'CUSTOM') {
    if (form.value.fixedMonth == null || form.value.fixedMonth === 0) {
      validationError.value = 'Bitte einen Monat (1–12) in Wiederkehrend angeben.'
      return
    }
    if (form.value.fixedDay == null || form.value.fixedDay === 0) {
      validationError.value = 'Bitte einen Tag (1–31) in Wiederkehrend angeben.'
      return
    }
  }
      ...
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
          ...
          <form @submit.prevent="handleSubmit" class="px-6 pb-6 space-y-4">
            ...
            <p v-if="validationError" class="mt-2 text-xs text-red-600 flex items-center gap-1">
              <svg
